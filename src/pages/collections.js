      ...
      <section className="bg-white py-8">
        ...
        <div className="container mx-auto flex items-center flex-wrap pt-4 pb-12">
            ...
            {collections.map(collection => {
              ...
              return (
              <div key={collection.id} className="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col">
              <Link to={`/collections/${collection.handle}`}>
                  {collection.image && <img className="hover:grow hover:shadow-lg" src={collection.image.originalSrc} />}
...
query MyQuery2 {
  allShopifyCollection (limit: 20, sort:{ fields: updatedAt, order: DESC }) {
    edges {
      node {
        id
        handle
